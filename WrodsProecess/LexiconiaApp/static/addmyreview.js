...
class AddWordsApp {
    ...
    initializeElements() {
        this.addWordsForm = document.getElementById('addWordsForm');
...
document.addEventListener('DOMContentLoaded', () => {
    // 初始化 Lexiconia 应用（如果在该页面）
    if (document.getElementById('word-card')) {
        const lexiconiaApp = new LexiconiaApp();
        lexiconiaApp.initializeCardsList();
    }
    
    // 初始化 Add Words 应用（如果在该页面）
    if (document.getElementById('addWordsForm')) {
        new AddWordsApp();
    }
});
// 初始化应用
