...
class Crawler:
    ...
    def crawl_byword(self, word: str):
        """
        Crawl a single word by its root
        
        Args:
            word (str): The word to crawl. word is insured to be exist in the word repository in previous process
        """
        url = self.search_api.format(word)
        response = requests.get(url, headers=self.headers)

        return self._parse_process(word, response.content.decode(response.apparent_encoding))            
    ...
    def _parse_and_save_word_info(self, root, html_content):
        ...
        if simple_dict_module:
            ...
            for i, word_exp in enumerate(word_exps, 1):
                ...
                details.append({
                    "root": "{:0>8d}".format(int(root)),
                    "serial": "19-{:0>8d}-00-{}".format(int(root), i),
                    "level": "9",
    ...
    def _parse_process(self, word: str, html_content):
        """解析HTML并返回详情"""
        soup = BeautifulSoup(html_content, 'html.parser')
        ...
        if simple_dict_module:
            ...
            for i, word_exp in enumerate(word_exps, 1):
                ...
                pos_text = pos.get_text(strip=True) if pos else "-"
                trans_text = trans.get_text(strip=True) if trans else "-"
                add = self._switch_pfs(pos_text, addition)
